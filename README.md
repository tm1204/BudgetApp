...
- **Tap a day** to see its date and amount as a toast — the grid also sets `title` for desktop/browser hover, but this is a phone-only app with no hover state, so tapping is the primary way to read a cell
