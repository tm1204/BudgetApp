...
- **Reorder categories** — press and hold the category's ⋮ button (the same one that opens its menu — a short tap still does that) until it lifts, then drag it to a new position and release; Income is permanently locked to the top position and can never be picked up or dropped onto
...
- **Tap a day** to expand an inline summary panel below the grid — shows the day's total plus every individual spend that made it up (expense name, category, amount, time), reusing the same row style as View Log. Tapping the same day again collapses it; the tapped cell gets a highlight ring. Narrowing the category filter while a day is open narrows the panel too, without closing it — comparing one day across categories stays possible. The grid also sets `title` for desktop/browser hover, but this is a phone-only app with no hover state, so the tap panel is the primary way to read a cell
...
| v5.10.1 | Bug fix — a plain swipe starting on a row/category (not a long press) scrolled the whole page as intended, but once a long press *did* fire, the browser had usually already committed to that native scroll and ignored the drag taking over, so the finger just kept scrolling the page instead of moving the row/category. Root cause: `touch-action` was only set to `none` after the long-press armed, but iOS Safari (and most mobile browsers) decide whether a touch will scroll at the moment it begins, using whatever `touch-action` was already declared — a change made mid-gesture is too late. Fixed by declaring `touch-action: none` on rows/category headers up front in CSS, and replacing the native swipe-scroll it now pre-empts with an equivalent manual `scrollBy()` during the press-and-hold window, so scrolling still works exactly as before for anything that isn't a long press |
| v5.10.2 | Bug fix — v5.10.1's manual swipe-scroll called `scrollBy()` straight from every raw `pointermove` event, which touch devices can deliver in uneven bursts out of sync with paint, making the scroll feel jerky/erratic rather than smooth. Batched it to apply at most once per animation frame instead — the same technique already used for auto-scroll during an active drag |
| v5.10.3 | Attempted fix — scrolling from a row/category was still reported as erratic after v5.10.2, unlike scrolling from any other part of the screen. The pointer wasn't captured until a drag actually armed, so while content was scrolling underneath a still-held finger during the pending phase, the browser was free to reassign which element the touch was targeting; now captured immediately on the row/category at the first touch, before any movement, pinning event delivery regardless of what scrolls past underneath. Not fully confirmed against a real device yet — manual JS-driven scroll inherently has more latency than native compositor scrolling, so this may not be the whole story |
| v5.11 | Moved the drag trigger from "long-press blank space anywhere on a row/category" to "long-press its ⋮ button" (a short tap still opens the usual menu) — the root cause of v5.10.1–v5.10.3's ongoing scroll issues was that `touch-action: none` had to cover the whole row/header to make dragging reliable, which also disabled that area's native swipe-to-scroll and forced a hand-rolled JS replacement that never fully matched native scroll feel. Scoping `touch-action: none` to just the small ⋮ button removes that trade-off entirely — the rest of the row/header keeps native scrolling untouched, and the manual-scroll fallback code is gone. Since the ⋮ button now does double duty (tap for menu, hold to drag), the drop handler defers its data-mutating re-render by one tick so a stray synthetic click after a drag reliably gets suppressed instead of reopening the menu |
| v5.11.1 | Bug fix — confirming the "new version available" prompt could reload straight back into the same old version and immediately re-prompt, over and over. The version.json check that triggers the prompt is a single small fetch that routinely resolves before the service worker's own update-and-install cycle (which has to download and cache every asset) has produced a worker to actually activate; reloading as soon as none was found yet landed on the same stale cached assets, re-triggering the same prompt. Fixed by waiting for that install to actually finish (or, if it hadn't even started yet, for it to start) before reloading, instead of treating "nothing to activate yet" as "nothing to activate" |
| v5.11.2 | Bug fix — dragging a row by its ⋮ button worked, but often triggered the native text-selection menu instead, since v5.11's `.row-ellipsis-btn` is an unpadded 18px glyph inside a 32px-wide grid column — a much smaller, easier-to-miss target than the category header's ⋮ button, especially holding still for a long press. A touch that drifted even slightly off it landed on plain row text, which has no long-press suppression. Enlarged the row ⋮ button's actual touch target well past its visible glyph via an invisible pseudo-element (biased toward the row's own padding and outer edge, kept tight toward the adjacent checkbox/running-total cell), without changing row height or the button's own layout box |
